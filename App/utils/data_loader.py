...
@st.cache_resource
def load_model():
    """Load the trained LightGBM model from its native text format.

    Native format (Booster.save_model) instead of joblib.dump of the sklearn
    wrapper — the latter pickles the exact class definition of whatever
    LightGBM/scikit-learn version was installed at export time, which breaks
    the moment the deployed environment resolves a different version.
    """
    import lightgbm as lgb
    model_path = MODELS_DIR / "lgbm_avm_model.txt"
