...
from utils.theme import CSS, render_footer, render_header
...
# every prediction, not just a bare number. Matches the constant of the
# same name on the About page.
MEDIAN_APE = 0.244

st.set_page_config(page_title="The Model — Real Estate Cost Intelligence", page_icon="🏗️", layout="wide")

st.markdown(CSS + render_header("THE MODEL"), unsafe_allow_html=True)
st.markdown('<p class="rec-page-title">Price Predictor — enter a project\'s details for a predicted cost per sqft, with the model\'s honest uncertainty band.</p>', unsafe_allow_html=True)
